...
def image_stream(datapath, use_depth=False, stride=1): #TODO adapt
    ...
    fx, fy, cx, cy = 362.4751201044906, 366.65683906579966, 249.93870791241568, 203.15151114146838
    
    image_list = sorted(glob.glob(os.path.join(datapath, "kinect_v2", "ir", "undistorted", '*.png')))[::stride]
    ...
    for t, (image_file, depth_file) in enumerate(zip(image_list, depth_list)):
        ...
        #Make RGB channels (from 2 to 3 dimensions)
        #Also normalize to 0-255
        image = np.stack((image/256.0,) * 3, axis=-1) #.astype(np.uint8)
...
if __name__ == '__main__':
    ...
    print(traj_est)
    ### run evaluation ### TODO needs trajectory, or make 3d evaluation
