...
# Like ETH3D for generated datasets, filtering depth by confidence (amplitude)
...
def show_image_depth(image):
    ...
    cv2.imshow('depth', image.astype(np.uint8))
    cv2.waitKey(1)
...
def image_stream(datapath, use_depth=False, stride=1, threshold=0):
    """ image generator """
    ...
    conf_list = sorted(glob.glob(os.path.join(datapath, "ir", '*.png')))[::stride]

    for t, (image_file, depth_file, conf_file) in enumerate(zip(image_list, depth_list, conf_list)):
        image = cv2.imread(image_file)
        depth = cv2.imread(depth_file, cv2.IMREAD_ANYDEPTH) / 5000.0
        confidence = cv2.imread(conf_file)[:, :, 0]
